...
class KakaoController < ApplicationController
  ...
  # 클래스 변수와 전역 변수.
  @@main_menu = [MENU_STEP_HOME,MENU_STEP_FIND_CANDI,MENU_STEP_SECOND]
  ...
  def message
    ...
    if @@users[user_key].nil?
        @mstep = MENU_STEP_HOME
        @fstep = 0
        @rem = Array.new
    else
        @rem = @@users[user_key][:rem]
        @mstep = @@users[user_key][:mstep]
        @fstep = @rem[-1]
    end
